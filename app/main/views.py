...
def index():
    ...
    if current_user.can(Permission.WRITE) and form.validate_on_submit():
        post = Post(body=form.body.data,
                    ...
                    post_tags = form.tags.data,
                    author=current_user._get_current_object())
        
        if form.body_format.data == 'html':
...
def followers(username):
    ...
    error_out=False)
    follows = [{'user': item.follower, 'timestamp': item.create_at}
...
def explore():
    ...
    if tag:
        like_pattern = f"%{tag}%"
        query = query.filter(Post.tags.ilike(like_pattern))

    pagination = query.order_by(Post.timestamp.desc()).paginate(page=page, per_page=10, error_out=False)
    posts = pagination.items
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        post.body_format = form.body_format.data

        if post.body_format in ('markdown', 'html'):
            post.body = None
            post.body_html = form.body.data
        else:
            post.body = form.body.data
        ...
        post.tags = getattr(form, 'tags', None)
