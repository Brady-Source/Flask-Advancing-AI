...
class User(UserMixin, db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.role is None:
            with db.session.no_autoflush:  # <-- add this
    ...
    def password(self, password_plain):
        self.password_salt = gen_salt(16)

        salted = (self.password_salt + password_plain).encode('utf-8')
        self.password_hash = hashlib.sha256(salted).hexdigest()

    def verify_password(self, password_plain):
        if not self.password_salt or not self.password_hash:
            return False
        salted = (self.password_salt + password_plain).encode('utf-8')
        candidate = hashlib.sha256(salted).hexdigest()
