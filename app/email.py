...
def send_email(to, subject, template, **kwargs):
    try:
        ...
        if not service:
            current_app.logger.error(f"Could not send email to {to}: Gmail service not initialized")
        ...
        current_app.logger.info(f"Email '{subject}' sent to {to}, template={template}, kwargs={kwargs}")
        current_app.logger.info(f"Email body being sent:\n{body}")
        return "Email sent"
    except Exception as e:
        current_app.logger.error(f"Failed to send email to {to}: {str(e)}, template={template}, kwargs={kwargs}")
